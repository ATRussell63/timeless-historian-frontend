...
export const ENGLISH_JEWEL_NAMES = ['Brutal Restraint', 'Elegant Hubris', 'Glorious Vanity', 'Lethal Pride', 'Militant Faith', 'Heroic Tragedy'];

const FRENCH_ANCHOR = 'Rareté: Unique'
const FRENCH_NAMES = ['Retenue brutale', 'Orgueil élégant', 'Vanité glorieuse', 'Fierté fatale', 'Foi militante', 'Tragédie héroïque'];

const GERMAN_ANCHOR = 'Seltenheit: Einzigartig'
const GERMAN_NAMES = ['Brutale Beherrschung', 'Heikler Übermut', 'Glorreiche Eitelkeit', 'Tödlicher Stolz', 'Militanter Glaube', 'Heroische Tragödie'];

const PORTUGESE_ANCHOR = 'Raridade: Único'
const PORTUGESE_NAMES = ['Restrição Brutal', 'Arrogância Elegante', 'Vaidade Gloriosa', 'Orgulho Letal', 'Fé Militante', 'Tragédia Heroica'];

const RUSSIAN_ANCHOR = 'Редкость: Уникальный'
const RUSSIAN_NAMES = ['Жестокая сдержанность', 'Изящный эготизм', 'Блистательное тщеславие', 'Смертельная гордость', 'Воинственная вера', 'Трагедия героев']

const SPANISH_ANCHOR = 'Rareza: Único'
const SPANISH_NAMES = ['Restricción brutal', 'Arrogancia elegante', 'Vanidad gloriosa', 'Orgullo letal', 'Fe combativa', 'Tragedia heroica']

const THAI_ANCHOR = 'ความหายาก: ยูนิค'
const THAI_NAMES = ENGLISH_JEWEL_NAMES

const KOREAN_ANCHOR = '아이템 희귀도: 고유'
const KOREAN_NAMES = ['잔인한 속박', '고상한 오만', '찬란한 허영심', '치명적인 긍지', '호전적인 신념', '비극적 영웅담']

const JAPANESE_ANCHOR = 'レアリティ: ユニーク'
const JAPANESE_NAMES = ['残忍な自制心', '上品な慢心', '栄光ある虚栄心', '致命的な自尊心', '戦闘的な信仰心', '英雄的悲劇']
